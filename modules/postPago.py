...
def updatePago(id):
    ...
    if(len(data)):
            ...
            peticion = requests.post(f"http://154.38.171.54:5006/pagos/{id}", data=json.dumps(pagos))
            res = peticion.json()
            res["Mensaje"]= "Pago Guardado"
            return [res]
    else:
...
""")
        ...
        try:
            ...
            if(opcion == 2):
                ...
                print(tabulate(deletePago(idClient)["body"], tablefmt="grid"))
                input("Presione una tecla para continuar......")
            if(opcion == 3):
                idClient = input("Ingrese el id del cliente que desea eliminar: ")
                print(tabulate(updatePago(idClient), tablefmt="grid"))
                input("Presione una tecla para continuar......")
            elif(opcion == 0):
                break
